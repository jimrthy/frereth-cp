...
(def client-keys
  {:public (byte-array [3 127 -40 124 49 16 61 -10 112 -26 -18 -49 26 0 -125 82 25 -127 -20 56 -57 -46 122 -61 -58 -50 121 38 54 13 -69 66]),
   :secret (byte-array [49 107 -101 43 -92 109 -109 -94 -99 119 -78 65 124 -20 63 96 43 70 54 30 31 -54 -3 7 -71 90 70 -112 120 45 66 -68])})

(def server-keys
  {:public (byte-array [-124 60 33 47 -58 -65 91 103 -57 -92 -61 -33 -77 80 -102 41 26 -5 -15 -78 25 82 -20 -77 90 1 107 16 21 125 109 109]),
   :secret (byte-array [-41 -106 17 -62 4 19 100 41 -128 -1 18 22 125 -4 -56 -57 -108 -5 45 108 110 30 -78 98 -92 94 122 22 -12 47 92 119])})
...
(deftest compare-inner-cookie-approaches
  ...
  (let [initial-log-state (log/init ::build-inner)
        ...
        {easier ::specs/byte-array} (cookie/build-inner-cookie initial-log-state
                                                               ...
                                                               minute-key
                                                               (byte-array working-nonce))
        easier' (vec (drop 16 (vec easier)))]
    (is (= original' easier'))
      ;; FIXME: Start back here:
      ;; Do they match if we discard the first 16 bytes of original?
