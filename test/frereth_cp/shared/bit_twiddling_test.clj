...
(deftest encrypted-long-pk

  (let [expected-v [0xA6 0x17 0xE1 0x3E 0x04 0xA9 0x90 0xBB
                    0x5D 0x2C 0x36 0x0A 0x14 0xD4 0xA2 0x66
                    0x50 0xD0 0x56 0x3B 0x41 0x35 0x57 0x60
                    0x7B 0x41 0x9C 0x6E 0x67 0x49 0x19 0x32]
        expected (byte-array expected-v)
        actual-v [0x4E 0xC3 0x2A 0x72 0x85 0x7F 0x86 0x64
                  0x45 0x86 0x34 0x64 0x5D 0x56 0x45 0x84
                  0x13 0xA0 0x99 0x2B 0x10 0x37 0x90 0x2A
                  0xBE 0x0A 0xFB 0x14 0x93 0xFD 0x9E 0xCD]
        actual (byte-array actual-v)]
    ;; actual is an encrypted version of expected. It seems nonsensical
    ;; that these two should compare successfully.
    ;; My translation has to be wrong.
    ;; Q: Doesn't it?!
    (is (not (b-t/bytes= expected actual)))))
